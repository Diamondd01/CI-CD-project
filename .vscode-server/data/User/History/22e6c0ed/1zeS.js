...
app.get('random-dish/:categories',async(req,res)=>{
  const {categories} = req.params;

  try{
    const result = await pool.query('SELECT * FROM Recipes WHERE category_id=(SELECT category_id FROM categories WHERE name= $1)', 
    [categories])
